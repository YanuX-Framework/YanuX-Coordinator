...
/**
 * Concrete implementation of the {@link SimpleCoordinator} interface that connects to the YanuX Broker using the Feathers Socket.io Client.
 */
class SimpleFeathersCoordinator implements SimpleCoordinator {
    protected static GENERIC_EVENT_CALLBACK: (evenType: string) => (event: any) => void
    ...
    /**
     * Constructor that creates a ready to use {@link SimpleFeathersCoordinator} instance.
     ...
     * A function that simply logs the event is provided by default.
     * @param localStorageLocation - The path where to save the local storage data when running the {@link SimpleFeathersCoordinator} on Node.js.
     */
    constructor(
        brokerUrl: string,
        ...
        if(!this.storage && window && window.localStorage) {
        ...
        this.feathersClient.on('authenticated', onAuthenticated ? onAuthenticated : SimpleFeathersCoordinator.GENERIC_EVENT_CALLBACK('authenticated'));
        this.feathersClient.on('logout', onLogout ? onLogout : SimpleFeathersCoordinator.GENERIC_EVENT_CALLBACK('logout'));
        this.feathersClient.on('reauthentication-error', onReAuthenticationError ? onReAuthenticationError : SimpleFeathersCoordinator.GENERIC_EVENT_CALLBACK('reauthentication-error'));
        ...
        return new Promise<any>((resolve, reject) => {
            this.feathersClient.authentication.getAccessToken().then((jwt: any) => {
                if (jwt) {
                    ...
                    } catch (e) { console.error('Access Token is not a JWT:', e) }
                }
                const auth: any = {};
    ...
    public subscribeResources(subscriberFunction: (data: any, eventType: string) => void): void {
        const eventListener = (resource: any, eventType: string = 'updated') => {
            ...
            if (this.client && this.client.id === resource.client &&
                ...
                SimpleFeathersCoordinator.cacheCleanup(this.cachedResources);
    ...
    public subscribeProxemics(subscriberFunction: (data: Proxemics, eventType: string) => void): void {
        const eventListener = (proxemics: any, eventType: string = 'updated') => {
            ...
            if (this.proxemics && this.proxemics.id === proxemics._id && (owner || sharedWith || prevSharedWith)) {
                ...
                SimpleFeathersCoordinator.cacheCleanup(this.cachedProxemics);
    ...
    public subscribeInstances(subscriberFunction: (data: Instance, eventType: string) => void): void {
        const eventListener = (instance: any, eventType: string = 'updated') => {
            ...
            if (this.client && this.client.id === instance.client && (owner || sharedWith || prevSharedWith)) {
                ...
                SimpleFeathersCoordinator.cacheCleanup(this.cachedInstances);
...
/**
 * Enforcing that the {@link CoordinatorConstructor} is being used by {@link SimpleFeathersCoordinator}.
 */
const FeathersCoordinatorClassExpression: SimpleCoordinatorConstructor = SimpleFeathersCoordinator;
export { SimpleFeathersCoordinator, FeathersCoordinatorClassExpression }
export default SimpleFeathersCoordinator;